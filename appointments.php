...
if (isset($_POST['search']) && !empty($_POST['search_query'])) {
  ...
  $sql = "SELECT appointment.*, doctor.doctor_name, Specialization
        FROM appointment 
        INNER JOIN doctor ON appointment.DoctorID = UserID 
        WHERE appointment.Status = 'Available' AND appointment_date LIKE '%$searchQuery%'";
} else {
  $sql = "SELECT appointment.*, Specialization, doctor.doctor_name
        FROM appointment 
        INNER JOIN doctor ON appointment.DoctorID = UserID 
    ...
    <div class="container-fluid">
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-6 d-flex justify-content-end gap-2">
          <a href="add-offer.php" class="btn btn-success add-offer-btn"><i class="fas fa-plus"></i> Add Appointment</a>
          <a href="scheduled-app.php" class="btn btn-success add-offer-btn"><i class="fas fa-eye"></i> View Scheduled
            Appointments</a>
          <a href="completed-app.php" class="btn btn-success add-offer-btn"><i class="fas fa-eye"></i> View Completed
            Appointments</a>
          <a href="canceled-app.php" class="btn btn-success add-offer-btn"><i class="fas fa-eye"></i> View Canceled
            Appointments</a>
        </div>
            ...
            <tbody>
              <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                  ...
                  <td><?= $row['Status'] ?></td>
                  <td><a href="edit-offer.php?id=<?= $row['AppointmentID'] ?>"><i class="fas fa-pencil-alt"></i></a></td>
      ...
      <?php else: ?>
        <div class="no-appointments text-center">
          <div class="no-appointments-message">
            ...
            <p>It looks like there are no appointments at the moment. <br> Click below to add a new appointment
              offer.</p>
            <a href="add-offer.php" class="btn btn-primary"><i class="fas fa-plus"></i> Add Appointment
              Offer</a>
          </div>
        </div>
      <?php endif; ?>
