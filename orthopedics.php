...
if (isset($_POST['search']) && !empty($_POST['search_query'])) {
  ...
  $sql_select = "SELECT * FROM appointment, doctor WHERE DoctorID = UserID AND Specialization = 'Orthopedics' AND status = 'Available' AND doctor.doctor_name LIKE '%$search_query%'";
} else {
  $sql_select = "SELECT * FROM appointment, doctor WHERE DoctorID = UserID AND Specialization = 'Orthopedics' AND status = 'Available'";
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Orthopedics Details and Appointment</title>
